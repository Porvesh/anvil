...
The default SQLite database and in-memory rate limiter support local or
single-instance use. A multi-instance deployment needs PostgreSQL migrations,
pooled connections, shared rate limiting, stable secrets, mail delivery, and a
separate worker with `python3`. The hosting proxy must support long-lived SSE
