...
function problemContext(body: string): string {
  return body;
}
...
export async function judgeReview(
  ...
  return structuredModelOutput(
    ...
    system,
    user,
    { headline: "Review graded", summary: "", assessments: [] },
    signal,
  );
}
...
export async function judgeDesign(client: ModelClient, problem: Problem, doc: string, signal?: AbortSignal): Promise<DesignJudgment> {
  const system = problemContext(
    [
